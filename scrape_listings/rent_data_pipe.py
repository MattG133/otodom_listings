...
LISTINGS_PATH = '/Users/Maciek/Desktop/Mieszkania/scrape_listings/otodom_listings/scrape_listings/data/raw/rent_listings_page.csv'
ITEM_PATH = '/Users/Maciek/Desktop/Mieszkania/scrape_listings/otodom_listings/scrape_listings/data/raw/rent_listings_item.csv'
...
def convert_to_integer(s):
    ...
    """
    if isinstance(s, str):
        # Remove non-numeric characters, including upper indices (e.g., "²")
...
def convert_to_float(s):
    ...
    '''
    s = s.replace(',', '.').replace(' ', '')
    numeric_str = ''.join(re.findall(r"[-+]?\d*\.\d+|\d+", s))
    if numeric_str:
...
# convert 'price', 'area', 'fees', 'deposit' columns to integer
df_full['rent'] = df_full['rent'].apply(convert_to_float)
df_full['area_y'] = df_full['area_y'].apply(convert_to_float)
df_full['fees'] = df_full['fees'].apply(convert_to_float)
df_full['deposit'] = df_full['deposit'].apply(convert_to_float)
...
# change deposit value to null if it's more than 3x the rent, cause that's an error
df_full.loc[df_full['deposit'] > 3*df_full['rent'], 'deposit'] = 'N/A'
...
# rename some cols
df_full = df_full.rename({"area_y": "area"}, axis=1)

# drop duplicates
df_full = df_full.drop_duplicates()

# save clean file to new dir
df_full.to_csv('../scrape_listings/data/clean/rent_listings_clean_{}.csv'.format(df_full['timestamp'].dt.strftime('%Y-%m-%d')[0]), index=False)
